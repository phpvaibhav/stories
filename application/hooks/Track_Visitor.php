...
class Track_Visitor {
    ...
    private $CONTROLLER_IGNORE_LIST = array(
        'admin','category','adminapi','apiv1','customers','pages','password','stories','item'
    );
    ...
    function visitor_track() {
        ...
        if (isset($track_visitor) && $track_visitor === TRUE) {
            ...
            if (in_array($this->get_client_ip(), $this->IP_IGNORE_LIST)) {
                $proceed = FALSE;
            }
            if ($proceed === TRUE) {
                $this->log_visitor();
        ...
        if ($this->track_session() === TRUE) {
            ...
            if (isset($temp_current_page) && $temp_current_page != $current_page) {
                ...
                $data = array(
                    ...
                    'requested_url' => rawurldecode($this->ci->input->server('REQUEST_URI')),
                    'referer_page' => rawurldecode($this->ci->agent->referrer()),
                    'user_agent' => rawurldecode($this->ci->agent->agent_string()),
